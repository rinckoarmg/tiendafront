...
const listaPr = async () => {
    try {
        let res = await fetch("http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/listar"),
            json = await res.json();
...
D.addEventListener("submit", async (e) => {
    if (e.target == $buscar) {
        ...
        e.preventDefault();
        try {
            let res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/buscar/${e.target.codigoProducto.value}`),
                json = await res.json();
            if (!res.ok) throw { status: res.status, statusText: res.statusText };
            console.log(json);
        ...
        if (borrar) {
            try {
                let datosPr = {
                    ...
                    res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/eliminar/${e.target.dataset.codigo_producto}`, datosPr),
                    json = await res.text();
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
            } catch (err) {
        ...
        if (!e.target.id.value) {
            ...
            try {
                let datosPr = {
                    ...
                    res = await fetch("http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/guardar", datosPr),
                    json = await res.json();
                console.log(res);
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
            ...
            try {
                let datosPr = {
                    ...
                    res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/actualizar/${e.target.InputCodigo.value}`, datosPr),
                    json = await res.json();
                console.log(res);
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
...
D.addEventListener("submit", async (e) => {
    if (e.target === $formulario) {
        e.preventDefault();
        try {
            let res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/buscar/${e.target.nitproveedor.value}`),
                json = await res.json();
            if (!res.ok) throw { status: res.status, statusText: res.statusText };
            console.log(json);
        } catch (err) {
...
function printFileContents(contents) {
    ...
    lines.forEach(async line => {
        ...
        try {
            let res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/buscar/${nit}`),
                json = await res.json();
            if (!res.ok) throw { status: res.status, statusText: res.statusText };
            console.log(json);
            ...
            try {
                let datosProv = {
                    ...
                    res = await fetch("http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/productos/guardar", datosProv);
                console.log(res);
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
            } catch (err) {
            ...
            alert("Por favor verifique los datos del proveedor del producto:" + " '" + nombre + "'");
            location.reload();
        }
    });
}