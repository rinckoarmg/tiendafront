...
const listaP = async () => {
    try {
        let res = await fetch("http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/listar"),
            json = await res.json();
        ...
        if (borrar) {
            try {
                let datosP = {
                    ...
                    res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/eliminar/${e.target.dataset.nitproveedor}`, datosP),
                    json = await res.text();
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
            } catch (error) {
        ...
        if (!e.target.id.value) {
            ...
            try {
                let datosP = {
                    ...
                    res = await fetch("http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/guardar", datosP),
                    json = await res.json();
                console.log(res);
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
            ...
            try {
                let datosP = {
                    ...
                    res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/proveedores/actualizar/${e.target.InputNit.value}`, datosP),
                    json = await res.json();
                console.log(res);
                if (!res.ok) throw { status: res.status, statusText: res.statusText };
                location.reload();
