...
async function buscarDetalleVentas() {
  ...
  try {
    let res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/detalle_ventas/buscar/${codDetalle}`),
      json = await res.json();
  ...
  if (e.target === $formulario) {
    ...
    try {
      let datosPr = {
        ...
        res = await fetch(`http://backend181-env.eba-wzp6p6pz.us-east-2.elasticbeanstalk.com/detalle_ventas/actualizar/${e.target.InputCodigo.value}`, datosPr),
        json = await res.json();
      console.log(res);
      if (!res.ok) throw { status: res.status, statusText: res.statusText };
      buscarDetalleVentas()
